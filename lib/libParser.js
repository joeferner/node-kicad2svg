...
module.exports = function(data, opts) {
  ...
  for (; i < lines.length; i++) {
    if (/^#.*encoding utf-8.*/.test(lines[i])) {
      comments = [];
      continue;
    }
    ...
    if (/^DEF\s+.*$/.test(lines[i])) {
      var startLine = i;
      for (; i < lines.length && lines[i].indexOf('ENDDEF') < 0; i++) {
      }
      var l = comments.concat(lines.slice(startLine, i + 1));
      var symbol = parseSymbolDef(l);
    ...
    throw new Error('Could not parse file ' + fname + ', invalid line: ' + (i + 1) + ': "' + lines[i] + '"');
  }
...
function parseSymbolDef(lines) {
  ...
  for (; i < lines.length; i++) {
    ...
    if (/^DEF.*$/.test(lines[i])) {
      ...
      symbol.name = defParts[1];
      symbol.prefix = defParts[2];
      symbol.numberOfPins = defParts[3];
      symbol.pinNameOffset = parseInt(defParts[4]);
      symbol.drawNums = defParts[5] !== 'N';
      symbol.drawName = defParts[6] !== 'N';
      symbol.unitCount = defParts[7];
      continue;
    }
