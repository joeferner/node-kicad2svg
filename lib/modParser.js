...
module.exports = function(data, opts) {
  ...
  for (; i < lines.length; i++) {
    ...
    if (/^\$MODULE\s+.*$/.test(lines[i])) {
      ...
      var module = parseModule(lines.slice(startLine, i));
  ...
  for (; i < lines.length; i++) {
    ...
    if (m = lines[i].match(/^T(\d+)\s+(.*)$/)) {
      ...
      var text = {
        ...
        value: stringUtils.trim(textParts[9], '"')
      };
      switch (m[1]) {
      case 0:
    ...
    if (/^D.*$/.test(lines[i])) {
      module.draw.push(parseDrawLine(lines[i]));
      continue;
    }
    ...
    if (lines[i] === "$SHAPE3D") {
      ...
      module.shape3d = lines.slice(startLine + 1, i - 1);
      continue;
    }
